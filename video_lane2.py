...
def get_const_information():
    ...
    M_image2undist = mpimg.imread('test_images/test10.jpg')
    origin_img_size = (
...
def get_M(img):
    ...
    src = np.float32([[670, 1072], [852, 697], [1260, 701], [1428, 1070]])
    # Four desired coordinates
    dst = np.float32([[670, 1072], [670, 600], [1428, 600], [1428, 1070]])
...
def process_image(image):
    ...
    binary_undist, stacked_binary = binary_transfor(image_cpy)
    ...
    if left_line.detected:
        ...
        result_img = project_lines(image_cpy, Minv, car_offset, warp_zero)
        return result_img
    else:
        print("False")
        return image_cpy
...
while(1):
    ...
    if ret:
        #frame_rgb = frame[:,:,::-1]
        #print(frame.shape)
        result_img = process_image(resized_frame)
...
'''
# local picture to test
while(1):
    image = mpimg.imread('test_images/test10.jpg')
    print(image.shape)
    image_cpy = np.copy(image)
    result_img = process_image(image_cpy)
